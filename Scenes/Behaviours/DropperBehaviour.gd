...
func drop_item() -> void:
	var rand_value  =rand_range(0.0, _compute_total_weight())
	var dropped_item = _find_item_by_weight_value(rand_value)
...
func _find_item_by_weight_value(value: float) -> Object:
	...
	push_error("The Given value %d must be between 0.0 and the total weight of item_drop_weight_array")
	return null
